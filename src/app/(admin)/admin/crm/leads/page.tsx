    ...
    <div>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button asChild variant="outline">
            <Link href="/admin/crm/leads/import">Import</Link>
          </Button>
          <Button asChild>
      ...
      <div className="mt-4 flex gap-2">
        <ViewTab href={`/admin/crm/leads${stripView(query)}`} active={params.view !== "all"}>
          Open
        </ViewTab>
        <ViewTab href={`/admin/crm/leads?${withView(query, "all")}`} active={params.view === "all"}>
          All statuses
        </ViewTab>
      </div>
          ...
          <div className="flex gap-2">
            {page > 1 ? (
              <Button asChild variant="outline" size="sm">
                <Link href={`/admin/crm/leads?${pageQuery(query, page - 1)}`}>Previous</Link>
              </Button>
            ) : null}
            {page < pageCount ? (
              <Button asChild variant="outline" size="sm">
                <Link href={`/admin/crm/leads?${pageQuery(query, page + 1)}`}>Next</Link>
              </Button>
            ) : null}
          </div>
        </div>
