...
/** `/admin/crm` — the consultancy's working view: what is open, what is due. */
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Button asChild variant="outline">
            <Link href="/admin/crm/students"><Users />Students</Link>
          </Button>
          <Button asChild>
            <Link href="/admin/crm/leads"><Inbox />Open leads</Link>
          </Button>
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        ...
        <section className="rounded-xl border bg-card p-5">
            ...
            {CRM_OPEN_STATUSES.concat(["converted", "lost", "not_interested"] as CrmLeadStatus[]).map(
              (status) => {
                ...
                return (
                  <li key={status}>
                    <Link
                      href={`/admin/crm/leads?status=${status}&view=all`}
          ...
          <Link
            href="/admin/crm/students?status=pending"
